...
boolean CountDownTimer()
{
  ...
  if (!Stop && !Paused) // if not Stopped or Paused, run timer
  {
    ...
    if ((_micro = micros()) - time > duration ) 
    {
      ...
      if (Clock == 0) // checks to see if the clock is 0
...
void ResetTimer() //you can stop the timer if any need to
{
  SetTimer(R_clock);
  Stop = false;
}
...
void StopTimer() //you can permanantly stop the timer
{
  Stop = true;
}
